...
#include "MPUlibraries/i2c_master.h"					/* Include I2C Mater header file */
#include "MPUlibraries/uart.h"							/* Include USART header file */

#define MPU6050_WRITE 0xD0
#define MPU6050_READ 0xD1

/*Creating Floating Variables for Gyro & Acceleration*/
float Acc_x;
float Acc_y;
float Acc_z;
float Gyro_x;
...
int main(void){
	...
	while(1){
		getreading();
		Xa = Acc_x/16384.0;								/* Divide raw value by sensitivity scale factor to get real values for "X","Y", "Z" */
		Ya = Acc_y/16384.0;
		Za = Acc_z/16384.0;
		Xg = Gyro_x/131;
		Yg = Gyro_y/131;
