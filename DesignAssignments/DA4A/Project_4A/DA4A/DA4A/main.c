...
volatile unsigned int adc_motor_speed;
char outs[20];
...
int main(void)
{
	...
	{	
		...
		adc_motor_speed = adc_motor_speed/4;
		snprintf(outs,sizeof(outs),"%3d\r\n", adc_motor_speed);  // print it
		USART_tx_string(outs); // Print the speed of the motor
		OCR2B = adc_motor_speed; // Replace with adc value
	}
}
...
void read_adc(void)
{
	...
	adc_motor_speed = adc_motor_speed / 4;  // Average a few samples
}
/* INIT USART (RS-232)  */
void USART_init( unsigned int ubrr )
{
