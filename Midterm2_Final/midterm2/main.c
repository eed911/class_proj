...
#define BRGVAL (F_CPU/16/BAUD) - 1
	...
	#define DEFAULT_PERS            0x11    // 2 consecutive proxy or ALS for int.
	#define DEFAULT_CONFIG2         0x01    // No saturation interrupts or LED boost
	#define DEFAULT_CONFIG3         0       // Enable all photo diodes, no SAI
...
int main(void){
	uint16_t red = 0, green = 0, blue = 0; //initialize variables red, green, blue
	...
	printf("AT+CWMODE=1\r\n");			//print AT+CWMODE=1\r\n	
	_delay_ms(5000);					//delay 5 seconds
	printf("AT+CWJAP=\"Michael iPhone\",\"hello dad\"\r\n");	//connect to michaels iphone wifi	
    while (1) {							// continuously send the data to thingspeak
		_delay_ms(5000);
		printf("AT+CIPMUX=0\r\n");
